...
const ProductDetails = () => {
  ...
  useEffect(() => {
    function fetchProductDetails() {
      const product = catalogs.categories.find(
  ...
  const products = catalogs.categories?.find(
    (category) => category.id === product_id!
  )?.products;
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{title} | Products | Ice Investment Nig Ltd</title>
      </Helmet>
      <NavWithContent title={product_id} image={bgImage} />
