...
const Navbar = ({}: NavbarProps) => {
  ...
  return (
    <nav
      className={`w-full duration-200 ease-in-out px-2 ${
        isFloating
      ...
      <div className="container mx-auto">
        <div className="flex justify-between lg:items-center border-gray-50">
          ...
          <div
            className={`${
              isMenuOpen
                ? "absolute lg:static w-full left-0 h-full -bottom-full lg:mt-0 mt-8 flex-1"
                : "hidden lg:block"
          ...
          <div className="space-x-2 hidden lg:block">
            {socialmedia.map((item, index) => (
              <Link
                target="_blank"
                to={item.link}
                className="rounded-full border border-[#ea6768] inline-block aspect-square p-2"
                key={index}
              >
                {item.icon}
              </Link>
